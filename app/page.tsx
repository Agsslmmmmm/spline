      ...
      <header id="home" className="flex flex-col-reverse md:flex-row w-full h-screen max-w-7xl items-center justify-center p-8 relative overflow-x-hidden">
        <div className='w-full h-2/4 md:h-full md:w-2/5 flex flex-col justify-center items-center md:items-start gap-8'>
          ...
          <div className='w-full flex items-center justify-center md:justify-start gap-4'>
            ...
            <button className='w-48 h-12 text-sm sm:text-base rounded hover:bg-fuchsia-800 hover:text-white hover:bg-opacity-50 transition-colors'>Contact</button>
          </div>
        </div>

        <div className='w-full h-2/4 md:h-full md:w-3/5 flex items-center justify-center relative -z-10'>
          <SplineNext className="w-full flex scale-[.25] sm:scale-[.35] lg:scale-[.5] items-center justify-center md:justify-start" scene='https://prod.spline.design/JGGvd5CQyJTgLu8e/scene.splinecode' />
        </div>
      ...
      <section id="pricing" className="h-fit min-h-screen w-full flex flex-col items-center justify-center gap-8 p-8">
        ...
        <div className="grid grid-cols-1 grid-rows-2 sm:grid-rows-1 sm:grid-cols-2 items-center h-fit w-full max-w-3xl gap-8">
          {pricingCards.map((pricingCard) => {
            return (
              <PricingCard oneliner={pricingCard.oneliner}
               title={pricingCard.title} 
               price={pricingCard.price} 
               benefits={pricingCard.benefits} key={pricingCard.id} />
            )
          })}
        </div>
      </section>
...
function InfoCard({title, Icon, children}:IInfoCardProps) {
  return (
    <div className="w-full h-80 rounded flex flex-col justify-around items-center p-8 bg-white rounded bg-clip-padding backdrop-filter backdrop-blur-xl bg-opacity-20">
...
function PricingCard({title, price, benefits, oneliner}:IPricingCardProps) {
  return (
    <div className="h-fit w-full rounded-flex flex-col p-8 gap-8 bg-white rounded bg-clip-padding backdrop-filter backdrop-blur-xl bg-opacity-20">
      <div className="flex flex-col gap-2">
        <div>
          <h6 className="text-2xl">{title}</h6>
          <p className="text-sm text-zinc-500">{oneliner}</p>
        </div>
        <p className="text-4xl font-bold">${price} <span className="text-sm font-normal text-zinc-500">/ Month</span></p>
      </div>
      ...
      <div className="flex flex-col w-full gap-4">
        {benefits.map((benefit, i) => {
          return (
            <p className="text-sm text-zinc-500 flex items-center gap-2" key={i}><span><CheckCheck /></span>{benefit}</p>
          )
        })}
      </div>
    </div>
